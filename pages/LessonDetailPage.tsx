...
const LessonDetailPage: React.FC = () => {
  ...
  // Sort files by order
  const sortedAudioFiles = [...(lesson.audioFiles || [])].sort((a, b) => a.order - b.order);
  const sortedPDFFiles = [...(lesson.pdfFiles || [])].sort((a, b) => a.order - b.order);
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-screen">
      ...
      <div className="bg-white dark:bg-bg-card-dark rounded-3xl shadow-sm border border-brand-grey-light dark:border-gray-700 overflow-hidden transition-colors duration-300">
        ...
        <div className="p-6 md:p-8 space-y-8">
            ...
            <>
              <div>
                <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
                  <Music className="text-brand-blue dark:text-sky-400" size={20} />
                  <span className="w-1.5 h-6 bg-brand-blue rounded-full"></span>
                  المقاطع الصوتية ({sortedAudioFiles.length})
                </h3>
                <div className="space-y-4">
                  {sortedAudioFiles.map((audioFile, index) => {
                    // Use the audio streaming URL function for better compatibility
                    const audioUrl = getGoogleDriveAudioStreamUrl(audioFile.google_drive_link);
                    return (
                      <div key={audioFile.id} className="bg-brand-blue-light/30 dark:bg-gray-800/50 p-4 rounded-2xl border border-transparent dark:border-gray-700">
                        <AudioPlayer 
                          src={audioUrl}
                          title={audioFile.title || `تسجيل صوتي ${index + 1}: ${lesson.title}`}
                        />
                        <div className="mt-2 text-xs text-brand-grey dark:text-gray-500 flex items-center gap-1">
                          <Info size={14} />
                          <span>يتم بث الصوت من Google Drive </span>
                        </div>
                      </div>
                    );
                  })}
          ...
          {sortedPDFFiles.length > 0 && (
            <div>
              <h3 className="text-lg font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
                ...
                <span className="w-1.5 h-6 bg-brand-blue rounded-full"></span>
                المواد الدراسية ({sortedPDFFiles.length})
              </h3>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {sortedPDFFiles.map((pdfFile) => {
                  const downloadUrl = getGoogleDriveDownloadLink(pdfFile.google_drive_link);
                  const viewUrl = convertGoogleDriveLink(pdfFile.google_drive_link, 'pdf');
                  ...
                  return (
                    <a
                      ...
                      className="flex items-center justify-between p-4 border border-brand-grey-light dark:border-gray-600 rounded-xl hover:border-brand-blue dark:hover:border-sky-500 hover:bg-brand-blue-light/20 dark:hover:bg-gray-700 transition-all group"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-right">
                          <div className="font-bold text-gray-700 dark:text-gray-200 group-hover:text-brand-blue dark:group-hover:text-sky-400">
                            {pdfFile.title}
                          </div>
                          <div className="text-xs text-brand-grey dark:text-gray-400">مستند PDF</div>
                        </div>
                      </div>
          ...
          {sortedAudioFiles.length === 0 && sortedPDFFiles.length === 0 && (
